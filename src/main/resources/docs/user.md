...
```
## 3. Метод getMyPosts (auth)
**логика метода**<br/>
сервис получает по id юзера его поста,
по id поста получает теги к каждому посту,
упорядочивает посты по времени от новым к старым и отсылает юзеру
